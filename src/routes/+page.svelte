<script lang="ts">
    ...
    let object = $state({back: false, selectedText: [selamatDatangtext[0], selamatDatangtext[1]]});
</script>
...
<div class="flex-col justify-center items-center text-center object-center w-screen h-fit">
    ...
    <div class="flex w-screen h-fit items-center justify-center text-center p-2 mt-20">
        <SelamatDatang sayText1={object.selectedText[0]} text2Say={object.selectedText[1]} text1Bold={true} text1Size="2rem" text2Size="1rem" class="text-white"/>
    </div>
            ...
            {#if !object.back}
                {#each buttonText as bText}
                    <button class={mergeClass1} onclick={() => {
                        ...
                        object.selectedText = [bText, "Masukan Akun"]
                    }}>
                        <p>{bText}</p>
                    </button>
                {/each}
